...
def grayplt(img,title=''):
    ...
    # Show the image
    if np.size(img.shape) == 3:
        ax.imshow(img[:,:,0],cmap='hot',vmin=0,vmax=np.max(img))
    else:
        ax.imshow(img,cmap='hot',vmin=0,vmax=np.max(img))
    print(title)
    plt.show()
...
for i in range(len(trDat)):
    temp=(trDat[i]-np.mean(trDat[i]))/np.std(trDat[i])
    trDat[i]=temp
    temp=trDat[i]/max(abs(np.max(trDat[i])),abs(np.min(trDat[i])) )
    trDat[i]=temp
for i in range(len(tsDat)):
    temp=(tsDat[i]-np.mean(tsDat[i]))/np.std(tsDat[i])
    tsDat[i]=temp
    temp=tsDat[i]/max(abs(np.max(tsDat[i])),abs(np.min(tsDat[i])) )
    tsDat[i]=temp
...
def createModel():
    ...
    x3 = Conv2D(16, (4,4), padding="same",strides=(4,4),kernel_initializer='he_normal', activation='relu')(x3)
    x3 = Conv2D(1, (4,4), padding="same",strides=(1,1),kernel_initializer='he_normal', activation='relu')(x3)
    x3 = UpSampling2D(size=(4, 4))(x3)
    ...
    x3a = Conv2D(16, (2,2), padding="same",strides=(2,2),kernel_initializer='he_normal', activation='relu')(x3a)
    x3a = Conv2D(1, (2,2), padding="same",strides=(1,1),kernel_initializer='he_normal', activation='relu')(x3a)
    x3a = UpSampling2D(size=(2, 2))(x3a)
    ...
    #x2 = add([x2,x2a])
    x2 = Conv2D(16, (10,1), padding="same",strides=(10,1),kernel_initializer='he_normal', activation='relu')(x2)
    x2 = Conv2D(1, (10,1), padding="same",strides=(1,1),kernel_initializer='he_normal', activation='relu')(x2)
    x2 = UpSampling2D(size=(10, 1), name='vertical')(x2)
    ...
    #x = multiply([x,inputs])
    #x = add([x,xa])
    x = Conv2D(16, (1,8), padding="same",strides=(1,8),kernel_initializer='he_normal', activation='relu')(x)
    x = Conv2D(1, (1,8), padding="same",strides=(1,1),kernel_initializer='he_normal', activation='relu')(x)
    x = UpSampling2D(size=(1, 8))(x)

    h = multiply([x,inputs], name="horizontal")
    h = Flatten()(h)
    h = Dense(64)(h)
    h = Dense(32)(h)
...
#i=0
    ...
    #print(tsDat2[i])
    #print(tsDat[i])
    grayplt(intermediate_output[i]*4,title='horizontal')
    #print(np.max(intermediate_output[i]),np.min(intermediate_output[i]))
    grayplt(intermediate_outputa[i]*4,title='vertical')
    grayplt(intermediate_outputb[i]*8,title='cluster')
    #grayplt(intermediate_output2[i]*4,title='pepper')
